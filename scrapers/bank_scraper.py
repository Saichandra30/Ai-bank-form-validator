...
class BankFormScraper:
    ...
    def save_policies(self, policies: List[Dict], filename: str = "bank_policies.json"):
        ...
        print(f"[SUCCESS] Saved {len(policies)} policies to {filepath}")
...
"""
        ...
        print(f"[SUCCESS] Created {len(policies)} policy documents")
    
    def scrape_all(self):
        """Main method to scrape and generate all policies"""
        print("[INFO] Starting bank policy generation...")
        
        # Generate comprehensive policies
        policies = self.generate_synthetic_policies()
        
        print(f"[INFO] Generated {len(policies)} policy documents")
